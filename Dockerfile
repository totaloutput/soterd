...
# src/github.com/soterium directory (not soterd directory).
...
#
# cd ~/src/github.com/soterium
# docker build --tag totaloutput/soterd:latest -f soterd/Dockerfile .
FROM golang:1.12.1

LABEL description="Soterium soterd image"
...
COPY ./soterd /go/src/github.com/totaloutput/soterd
COPY ./soterwallet /go/src/github.com/soterium/soterwallet

# TODO(cedric): Once we migrate from glide to go.mod we won't need this step
# Create a fresh go.mod file for soterd
WORKDIR /go/src/github.com/totaloutput/soterd
RUN rm --force go.mod go.sum; go mod init github.com/totaloutput/soterd
...
RUN go mod edit -replace=github.com/totaloutput/soterd=/go/src/github.com/totaloutput/soterd /go/src/github.com/totaloutput/soterd/go.mod

RUN go mod edit -replace=github.com/totaloutput/soterd=/go/src/github.com/totaloutput/soterd /go/src/github.com/soterium/soterwallet/go.mod
RUN go mod edit -replace=github.com/soterium/soterwallet=/go/src/github.com/soterium/soterwallet /go/src/github.com/soterium/soterwallet/go.mod
...
RUN /bin/bash -c 'hash=$(git log --max-count=1 --format=format:"%H" | cut -c 1-12); \
                  ...
                  ver="v0.0.0"; \
                  modVer="${ver}-${dt}-${hash}"; \
                  go mod edit -droprequire=github.com/totaloutput/soterd /go/src/github.com/soterium/soterwallet/go.mod; \
                  go mod edit -require=github.com/totaloutput/soterd@$modVer /go/src/github.com/soterium/soterwallet/go.mod'

WORKDIR /go/src/github.com/totaloutput/soterd
RUN go build && go install . ./cmd/... && mkdir -p /root/.soterd && touch /root/.soterd/soterd.conf

WORKDIR /go/src/github.com/soterium/soterwallet
