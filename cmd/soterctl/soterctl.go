...
func main() {
	...
	method := args[0]
	usageFlags, err := soterjson.MethodUsageFlags(method)
	if err != nil {
	...
	cmd, err := soterjson.NewCmd(method, params...)
	if err != nil {
		// Show the error along with its error code when it's a
		// soterjson.Error as it realistically will always be since the
		// NewCmd function is only supposed to return errors of that
		// type.
		if jerr, ok := err.(soterjson.Error); ok {
		...
		// The error is not a soterjson.Error and this really should not
	...
	marshalledJSON, err := soterjson.MarshalCmd(1, cmd)
	if err != nil {
