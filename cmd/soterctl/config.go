...
const (
	...
	unusableFlags = soterjson.UFWebsocketOnly | soterjson.UFNotification
)

var (
	soterdHomeDir         = soterutil.AppDataDir("soterd", false)
	soterctlHomeDir       = soterutil.AppDataDir("soterctl", false)
	soterwalletHomeDir    = soterutil.AppDataDir("soterwallet", false)
	defaultConfigFile     = filepath.Join(soterctlHomeDir, "soterctl.conf")
	defaultRPCServer      = "localhost"
	defaultRPCCertFile    = filepath.Join(soterdHomeDir, "rpc.cert")
	defaultWalletCertFile = filepath.Join(soterwalletHomeDir, "rpc.cert")
)
...
func listCommands() {
	...
	cmdMethods := soterjson.RegisteredCmdMethods()
	categorized := make([][]string, numCategories)
	for _, method := range cmdMethods {
		flags, err := soterjson.MethodUsageFlags(method)
		if err != nil {
		...
		usage, err := soterjson.MethodUsageText(method)
		if err != nil {
			// This should never happen since the method was just
			// returned from the package, but be safe.
			continue
		...
		if flags&soterjson.UFWalletOnly != 0 {
...
// config defines the configuration options for soterctl.
//
...
type config struct {
	...
	TestNet1      bool   `long:"testnet" description:"Connect to testnet"`
...
func normalizeAddress(addr string, useTestNet1, useSimNet, useWallet bool) string {
...
func cleanAndExpandPath(path string) string {
	...
	if strings.HasPrefix(path, "~") {
		homeDir := filepath.Dir(soterctlHomeDir)
...
func loadConfig() (*config, []string, error) {
	...
	if _, err := os.Stat(preCfg.ConfigFile); os.IsNotExist(err) {
		// Use config file for RPC server to create default soterctl config
		var serverConfigPath string
		if preCfg.Wallet {
			serverConfigPath = filepath.Join(soterwalletHomeDir, "soterwallet.conf")
		} else {
			serverConfigPath = filepath.Join(soterdHomeDir, "soterd.conf")
		}
	...
	// if needed.
	cfg.RPCServer = normalizeAddress(cfg.RPCServer, cfg.TestNet1,
...
// For this it tries to read the config file for the RPC server (either soterd or
// soterwallet), and extract the RPC user and password from it.
