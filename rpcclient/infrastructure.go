...
// Client represents a Soter RPC client which allows easy access to the
// various RPC methods available on a Soter RPC server.  Each of the wrapper
...
func (c *Client) trackRegisteredNtfns(cmd interface{}) {
	...
	case *soterjson.NotifyBlocksCmd:
		c.ntfnState.notifyBlocks = true

	case *soterjson.NotifyNewTransactionsCmd:
...
type (
	...
	rawResponse struct {
		...
		Error  *soterjson.RPCError `json:"error"`
	}
)
...
// returning an unmarshaled soterjson.RPCError (or an unmarshaling error) if so.
...
func (c *Client) reregisterNtfns() error {
	...
	if nslen > 0 {
		outpoints := make([]soterjson.OutPoint, 0, nslen)
