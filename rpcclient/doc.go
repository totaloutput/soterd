...
/*
Package rpcclient implements a websocket-enabled soter JSON-RPC client.
...
The section below discusses the differences between HTTP POST and websockets.
...
soterd or soterwallet by default.  However, configuration options are provided to
...
In contrast, the websocket-based JSON-RPC interface provided by soterd and
soterwallet only uses a single connection that remains open and allows
...
All notifications provided by soterd require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by soterd (and
soterwallet passthrough).  Therefore if you call one of these commands against
...
Also, it is important to realize that soterd intentionally separates the wallet
functionality into a separate process named soterwallet.  This means if you are
connected to the soterd RPC server directly, only the RPCs which are related to
chain services will be available.  Depending on your application, you might only
need chain-related RPCs.  In contrast, soterwallet provides pass through treatment
...
detected by type asserting the error in a *soterjson.RPCError.  For example, to
detect if a command is unimplemented by the remote RPC server:

  amount, err := client.GetBalance("")
  if err != nil {
  	if jerr, ok := err.(*soterjson.RPCError); ok {
  		switch jerr.Code {
  		case soterjson.ErrRPCUnimplemented:
...
The following full-blown client examples are in the examples directory:
 ...
 - soterdwebsockets
   ...
   block count
 - soterwalletwebsockets
   Connects to a soterwallet RPC server using TLS-secured websockets, registers
