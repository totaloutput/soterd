...
func TestDAGStateSerialization(t *testing.T) {
	...
	hashes[0] = *newHashFromStr("0001")
	hashes[1] = *newHashFromStr("000a")
	state := dagState{
		blkCount:  2,
		tipHashes: hashes,
	}

	data := serializeDAGState(state)
	dState, err := deserializeDAGState(data)

	if err != nil {
		t.Errorf("Error deserializing dag state: %v", err)
	}

	if dState.blkCount != 2 {
		t.Errorf("Expecting block count %d, got %d", 2, dState.blkCount)
	}

	for i, tipHash := range dState.tipHashes {
		if !tipHash.IsEqual(&hashes[i]) {
			t.Errorf("Expecting hash %v, got %v", hashes[i], tipHash)
		}
	}
}
...
func TestBlockNodeStorage(t *testing.T) {
	...
	if err != nil {
		t.Errorf("Error serializing block node: %v", err)
	}

	header, parents, status, err := deserializeBlockRow(blockData)
	if err != nil {
		t.Errorf("Error deserializing block node: %v", err)
	}

	expectedHash := msgblock1.BlockHash()
	hash := header.BlockHash()
	if !expectedHash.IsEqual(&hash) {
		t.Errorf("Error deserializing block header, expected hash %v got %v", expectedHash, hash)
	}

	if parents.Version != msgblock1.Parents.Version {
		t.Errorf("Error deserializing parent header, expected version %d, got %d",
			parents.Version, msgblock1.Parents.Version)
	}
	if parents.Size != msgblock1.Parents.Size {
		t.Errorf("Error deserializing parent header, expected size %d, got %d",
			parents.Size, msgblock1.Parents.Size)
	}

	for i, parent := range parents.Parents {
		if !parent.Hash.IsEqual(&msgblock1.Parents.Parents[i].Hash) {
			t.Errorf("Error deserializing parent header, expected parent %v, got %v",
				parent.Hash, msgblock1.Parents.Parents[i].Hash)
		}
	}
