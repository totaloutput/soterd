...
const (
	...
	// tfIgnore indicates that this txout is now invalid and should be ignore
	tfIgnored
)
...
// IsIgnored returns whether or not the output should be ignored
func (entry *UtxoEntry) IsIgnored() bool {
	return entry.packedFlags&tfIgnored == tfIgnored
}

// Marks entry to be ignored
func (entry *UtxoEntry) Ignore() {
	if entry.IsIgnored() {
		return
	}

	entry.packedFlags |= tfIgnored | tfModified
}
...
func (view *UtxoViewpoint) AddTxOut(tx *soterutil.Tx, txOutIdx uint32, blockHeight int32) {
...
func (view *UtxoViewpoint) AddTxOuts(tx *soterutil.Tx, blockHeight int32) {
...
func (view *UtxoViewpoint) addTxIgnoredOut(outpoint wire.OutPoint, txOut *wire.TxOut, blockHeight int32) {
	if txscript.IsUnspendable(txOut.PkScript) {
		return
	}
	...
	entry.pkScript = txOut.PkScript
	entry.blockHeight = blockHeight
	entry.packedFlags = tfModified | tfIgnored
}

func (view *UtxoViewpoint) AddTxIgnoredOuts(tx *soterutil.Tx, blockHeight int32) {
	prevOut := wire.OutPoint{Hash: *tx.Hash()}
	for txOutIdx, txOut := range tx.MsgTx().TxOut {
		prevOut.Index = uint32(txOutIdx)
		view.addTxIgnoredOut(prevOut, txOut, blockHeight)
	}
}
...
func (view *UtxoViewpoint) connectTransaction(tx *soterutil.Tx, blockHeight int32, stxos *[]SpentTxOut) error {
	...
	for _, txIn := range tx.MsgTx().TxIn {
		...
		if entry == nil {
			return AssertError(fmt.Sprintf("connectTransaction: view missing input %v",
...
func (view *UtxoViewpoint) connectTransactionForSorting(tx *soterutil.Tx, blockHeight int32, stxos *[]SpentTxOut) error {
	...
	toSpend := make([]*UtxoEntry, 0)

	// Spend the referenced utxos by marking them spent in the view and,
	// if a slice was provided for the spent txout details, append an entry
	// to it.
	for _, txIn := range tx.MsgTx().TxIn {
		// If entry is already spent or does not exist, mark as ignore and continue
		entry := view.entries[txIn.PreviousOutPoint]
		...
		// If entry is already spent or does not exist,
		// mark outputs as ignore and return
		if entry.IsIgnored() || entry.IsSpent() {

			view.AddTxIgnoredOuts(tx, blockHeight)
			return nil
		}


		// It is possible one of the inputs is a dup, or is ignored due to a dup upstream
		// only mark as transactions as spent once all inputs are valid
		toSpend = append(toSpend, entry)
	}

	// Transactions inputs are all valid
	// Spend them and add the outputs to UTXO
	for _, entry := range toSpend {
		if stxos != nil {
			var stxo = SpentTxOut{
				Amount:     entry.Amount(),
				PkScript:   entry.PkScript(),
...
func (view *UtxoViewpoint) connectTransactionsForSorting(block *soterutil.Block, stxos *[]SpentTxOut,
	chainParams *chaincfg.Params) error {

	//fmt.Println("START view connectTransactionForSorting")
	//for k,v := range view.entries {
	//	fmt.Printf("view outpoint hash: %v, index: %d, spent: %v, ignored: %v\n", k.Hash, k.Index, v.IsSpent(), v.IsIgnored())
	//}

	for _, tx := range block.Transactions() {
		_, err := CheckTransactionInputs(tx, block.Height(), view, chainParams, false)
		if err != nil {
			// skip over invalid txs
			continue
		}
		err = view.connectTransactionForSorting(tx, block.Height(), stxos)
		if err != nil {
			return err
		}
	}
	...
	//fmt.Println("END view connectTransactionForSorting")
	//for k,v := range view.entries {
	//	fmt.Printf("view outpoint hash: %v, index: %d, spent: %v, ignored: %v\n", k.Hash, k.Index, v.IsSpent(), v.IsIgnored())
	//}
...
func (view *UtxoViewpoint) connectTransactions(block *soterutil.Block, stxos *[]SpentTxOut) error {
...
func (view *UtxoViewpoint) disconnectTransactions(db database.DB, block *soterutil.Block, stxos []SpentTxOut) error {
...
func (view *UtxoViewpoint) commit() {
	for outpoint, entry := range view.entries {
		if entry == nil || (entry.isModified() && (entry.IsSpent() || entry.IsIgnored())) {
			delete(view.entries, outpoint)
			continue
		}
...
func (b *BlockDAG) FetchUtxoView(tx *soterutil.Tx) (*UtxoViewpoint, error) {
