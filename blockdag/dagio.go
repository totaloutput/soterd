...
var (
	...
	// dagStateKeyName is the name of the db key used to store the dag state.
...
func (b *BlockDAG) FetchSpendJournal(targetBlock *soterutil.Block) ([]SpentTxOut, error) {
	b.chainLock.RLock()
...
func dbFetchSpendJournalEntry(dbTx database.Tx, block *soterutil.Block) ([]SpentTxOut, error) {
...
func dbPutUtxoView(dbTx database.Tx, view *UtxoViewpoint) error {
	...
	for outpoint, entry := range view.entries {
		//fmt.Printf("tx hash in utxo view loop: %v\n", outpoint.Hash)
...
// -----------------------------------------------------------------------------
// DAG state contains the hash of the current tips
//
// The serialized format is:
//
//   <number of tips><tip hash 1>...<tip hash n>
//
//   Field             Type             Size
//   block count       uint32           4 bytes
//   number of tips    uint8            1 byte
//   block hash        chainhash.Hash   chainhash.HashSize
// -----------------------------------------------------------------------------

type dagState struct {
	blkCount  uint32
	tipHashes []chainhash.Hash
}

func serializeDAGState(state dagState) []byte {
	numTips := len(state.tipHashes)
	serializedLen := 4 + 1 + numTips * chainhash.HashSize

	serializedData := make([]byte, serializedLen)
	offset := 0
	byteOrder.PutUint32(serializedData[offset:], state.blkCount)
	offset += 4
	serializedData[offset] = uint8(numTips)
	offset += 1
	for _, hash := range state.tipHashes {
		copy(serializedData[offset:], hash[:])
...
func deserializeDAGState(serializedData []byte) (dagState, error) {
	...
	offset := 0
	state.blkCount = byteOrder.Uint32(serializedData[offset : offset+4])
	offset += 4
	numTips := int(serializedData[offset])
	offset += 1
	// check that rest of bytes is multiple of hash size
	if len(serializedData) < 1 + numTips * chainhash.HashSize {
	...
	for i := 0; i < numTips; i++ {
		copy(tips[i][:], serializedData[offset:offset + chainhash.HashSize])
		offset += chainhash.HashSize
	}
	state.tipHashes = tips
...
func dbPutDAGState(dbTx database.Tx, state *DAGState) error {
	serializedData := serializeDAGState(dagState{
		blkCount: state.BlkCount,
		tipHashes: state.Tips,
	})
...
func (b *BlockDAG) createChainState() error {
	...
	genesisBlock := soterutil.NewBlock(b.chainParams.GenesisBlock)
	...
	// Add genesis block to graph
	b.graph.AddNodeById(genesisBlock.Hash().String())
	...
	b.dagSnapshot = newDAGState(b.dView.Tips(), 1)
	...
	err := b.db.Update(func(dbTx database.Tx) error {
		...
		// Store the current dagstate into the database.
		err = dbPutDAGState(dbTx, b.dagSnapshot)
		if err != nil {
			return err
		}
...
func (b *BlockDAG) initChainState() error {
	...
	//if !hasBlockIndex {
	//	err := migrateBlockIndex(b.db) //TODO: for upgrading, comment out?
	//	if err != nil {
	//		return nil
	//	}
	//}
	...
	err = b.db.View(func(dbTx database.Tx) error {
		...
		dagState, err := deserializeDAGState(serializedData)
		if err != nil {
			return err
		}
		...
		cursor = blockIndexBucket.Cursor()
		for ok := cursor.First(); ok; ok = cursor.Next() {
			header, parentSubHeader, status, err := deserializeBlockRow(cursor.Value())
			if err != nil {
				return err
			}
			...
			//	parent = lastNode
				...
				for _, parent := range parentSubHeader.Parents {
					parentNode := b.index.LookupNode(&parent.Hash)
					if parentNode == nil {
			...
			node := &blockNodes[i]
			initBlockNode(node, header, parentSubHeader, parentNodes)
			node.status = status
			b.index.addNode(node)

			// Add block node to graph
			hashStr := node.hash.String()
			b.graph.AddNodeById(hashStr)
			for _, parentNode := range parentNodes {
				parentHashStr := parentNode.hash.String()
				if b.graph.GetNodeById(parentHashStr) == nil {
					b.graph.AddNodeById(parentHashStr)
				}
				b.graph.AddEdgeById(hashStr, parentHashStr)
			}
		...
		// set tips based on dag state
		for _, tip := range dagState.tipHashes {
			tipNode := b.index.LookupNode(&tip)
			if tipNode == nil {
				return AssertError(fmt.Sprintf("initChainState: cannot find "+
					"dag tip %s in block index", tip))
			}
			b.dView.AddTip(tipNode)
		}
		...
		blockWeight := uint64(GetBlockWeight(soterutil.NewBlock(&block)))
...
func deserializeBlockRow(blockRow []byte) (*wire.BlockHeader, *wire.ParentSubHeader, blockStatus, error) {
	...
	var parentSubHeader wire.ParentSubHeader
	err = parentSubHeader.Deserialize(buffer)
	if err != nil {
		return nil, nil, statusNone, err
	}
	...
	return &header, &parentSubHeader, blockStatus(statusByte), nil
}
...
// raw block for the provided node, deserialize it, and return a soterutil.Block
// with the height set.
func dbFetchBlockByNode(dbTx database.Tx, node *blockNode) (*soterutil.Block, error) {
...
func serializeBlockNode(node *blockNode) ([]byte, error) {
	...
	parentHeaderSize := wire.ParentVersionSize + wire.ParentCountSize + (wire.ParentSize * len(node.parents))

	w := bytes.NewBuffer(make([]byte, 0, blockHdrSize + parentHeaderSize + 1))
	header := node.Header()
	...
	parentSubHeader := node.ParentSubHeader()
	err = parentSubHeader.Serialize(w)
	if err != nil {
		return nil, err
	}
...
func dbStoreBlockNode(dbTx database.Tx, node *blockNode) error {
	...
	value, err := serializeBlockNode(node)
	if err != nil {
		return err
	}
...
func dbStoreBlock(dbTx database.Tx, block *soterutil.Block) error {
...
//
// This function is safe for concurrent access.
func (b *BlockDAG) BlocksByHeight(blockHeight int32) ([]*soterutil.Block, error) {
	...
	var blocks = make([]*soterutil.Block, 0, len(nodes))
	var err error
...
//
// This function is safe for concurrent access.
func (b *BlockDAG) BlockByHash(hash *chainhash.Hash) (*soterutil.Block, error) {
