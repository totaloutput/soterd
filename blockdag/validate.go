...
const (
	...
	baseSubsidy = 50 * soterutil.NanoSoterPerSoter
)
...
func IsFinalizedTransaction(tx *soterutil.Tx, blockHeight int32, blockTime time.Time) bool {
		msgTx := tx.MsgTx()
...
func CheckTransactionSanity(tx *soterutil.Tx) error {
	...
	// as a nanoSoter.  One soter is a quantity of nanoSoter as defined by the
	// NanoSoterPerSoter constant.
	var totalNanoSoter int64
	for _, txOut := range msgTx.TxOut {
		nanoSoter := txOut.Value
		if nanoSoter < 0 {
			str := fmt.Sprintf("transaction output has negative "+
				"value of %v", nanoSoter)
			return ruleError(ErrBadTxOutValue, str)
		}
		if nanoSoter > soterutil.MaxNanoSoter {
			str := fmt.Sprintf("transaction output value of %v is "+
				"higher than max allowed value of %v", nanoSoter,
				soterutil.MaxNanoSoter)
			return ruleError(ErrBadTxOutValue, str)
		}

		// Two's complement int64 overflow guarantees that any overflow
		// is detected and reported.  This is impossible for soter, but
		// perhaps possible if an alt increases the total money supply.
		totalNanoSoter += nanoSoter
		if totalNanoSoter < 0 {
		...
		if totalNanoSoter > soterutil.MaxNanoSoter {
			str := fmt.Sprintf("total value of all transaction "+
				"outputs is %v which is higher than max "+
				"allowed value of %v", totalNanoSoter,
				soterutil.MaxNanoSoter)
			return ruleError(ErrBadTxOutValue, str)
		}
	}
...
// txscript.
func CountSigOps(tx *soterutil.Tx) int {
	msgTx := tx.MsgTx()
...
func CountP2SHSigOps(tx *soterutil.Tx, isCoinBaseTx bool, utxoView *UtxoViewpoint) (int, error) {
...
func checkSerializedHeight(coinbaseTx *soterutil.Tx, wantHeight int32) error {
...
func (b *BlockDAG) checkBlockHeaderContext(header *wire.BlockHeader, prevNodes []*blockNode, flags BehaviorFlags) error {
	...
	if !fastAdd {
		...
		medianTime := recentTip.CalcPastMedianTime()
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockDAG) checkBlockContext(block *soterutil.Block, prevNodes []*blockNode, flags BehaviorFlags) error {
	// Perform all block header related validation checks.
	header := &block.MsgBlock().Header
	err := b.checkBlockHeaderContext(header, prevNodes, flags)
	if err != nil {
		return err
	}
...
func (b *BlockDAG) checkBIP0030(node *blockNode, block *soterutil.Block, view *UtxoViewpoint) error {
	...
	for outpoint := range fetchSet {
		...
		if utxo != nil { // && !utxo.IsSpent() { //DAG OK with dups
...
// the soter tokens and therefore allowed to spend them.  As it checks the inputs,
...
func CheckTransactionInputs(tx *soterutil.Tx, txHeight int32, utxoView *UtxoViewpoint, chainParams *chaincfg.Params,
	...
	for txInIndex, txIn := range tx.MsgTx().TxIn {
		...
		// soter is a quantity of nanoSoter as defined by the
		// NanoSoterPerSoter constant.
		originTxNanoSoter := utxo.Amount()
		if originTxNanoSoter < 0 {
			str := fmt.Sprintf("transaction output has negative "+
				"value of %v", soterutil.Amount(originTxNanoSoter))
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		if originTxNanoSoter > soterutil.MaxNanoSoter {
			str := fmt.Sprintf("transaction output value of %v is "+
				"higher than max allowed value of %v",
				soterutil.Amount(originTxNanoSoter),
				soterutil.MaxNanoSoter)
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		...
		lastNanoSoterIn := totalNanoSoterIn
		totalNanoSoterIn += originTxNanoSoter
		if totalNanoSoterIn < lastNanoSoterIn ||
			totalNanoSoterIn > soterutil.MaxNanoSoter {
			str := fmt.Sprintf("total value of all transaction "+
				"inputs is %v which is higher than max "+
				"allowed value of %v", totalNanoSoterIn,
				soterutil.MaxNanoSoter)
			return 0, ruleError(ErrBadTxOutValue, str)
		}
	}
	...
	var totalNanoSoterOut int64
	for _, txOut := range tx.MsgTx().TxOut {
		totalNanoSoterOut += txOut.Value
	}

	// Ensure the transaction does not spend more than its inputs.
	if totalNanoSoterIn < totalNanoSoterOut {
		str := fmt.Sprintf("total value of all transaction inputs for "+
			"transaction %v is %v which is less than the amount "+
			"spent of %v", txHash, totalNanoSoterIn, totalNanoSoterOut)
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockDAG) checkConnectBlock(node *blockNode, block *soterutil.Block, view *UtxoViewpoint, stxos *[]SpentTxOut) error {
	...
	*/
	...
	for i, tx := range transactions {
		...
		//	enforceSegWit)
		sigOpCost, err := GetSigOpCost(tx, i == 0, view, false,
			false)
		if err != nil {
			return err
		}
	...
	if totalNanoSoterOut > expectedNanoSoterOut {
		str := fmt.Sprintf("coinbase transaction for block pays %v "+
			...
			totalNanoSoterOut, expectedNanoSoterOut)
	...
	*/
	...
	view.SetBestHash(generateTipsHash(node.parents))
...
func (b *BlockDAG) CheckConnectBlockTemplate(block *soterutil.Block) error {
	b.chainLock.Lock()
	...
	// current chain.
	tips := b.dView.Tips()
	virtualHash := generateTipsHash(tips)
	header := block.MsgBlock().Header
	if *virtualHash != header.PrevBlock {
		str := fmt.Sprintf("previous block must be the current tips %v, "+
			"instead got %v", virtualHash, header.PrevBlock)
	...
	err = b.checkBlockContext(block, tips, flags)
	if err != nil {
		return err
	}
	...
	view.SetBestHash(virtualHash)

	newNode := newBlockNode(&header, &block.MsgBlock().Parents, tips)
