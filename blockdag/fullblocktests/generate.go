...
var (
	...
	// lowFee is a single nanoSoter and exists to make the test code more
	// readable.
	lowFee = soterutil.Amount(1)
)
...
type ExpectedTips struct { // FOR DAG
	tips []*ExpectedTip
}

// Ensure ExpectedTip implements the TestInstance interface.
var _ TestInstance = ExpectedTips{}

// FullBlockTestInstance only exists to allow ExpectedTip to be treated as a
// TestInstance.
//
// This implements the TestInstance interface.
func (b ExpectedTips) FullBlockTestInstance() {}
...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: soterutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func makeTestGenerator(params *chaincfg.Params) (testGenerator, error) {
	...
	return testGenerator{
		...
		lastName:     "genesis",
		lastHeight:   0,
		//tips:         []*wire.MsgBlock{genesis},
		tipNames:     []string{"genesis"},
		privKey:      privKey,
	}, nil
}
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := soterutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	merkles := blockdag.BuildMerkleTreeStore(utilTxns, false)
...
func additionalCoinbase(amount soterutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee soterutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee soterutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee soterutil.Amount) *wire.MsgTx {
...
func (g *testGenerator) nextBlock(blockName string, spend *spendableOut, parentNames []string, mungers ...func(*wire.MsgBlock)) *wire.MsgBlock {
	// Create coinbase transaction for the block using any additional
	// subsidy if specified.

	if parentNames == nil {
		parentNames = g.tipNames
	}

	maxHeight := g.lastHeight
	for _, tip := range parentNames {
		tipHeight := g.blockHeights[tip]
	...
	if nextHeight == 1 {
		...
		ts = g.last.Header.Timestamp.Add(time.Second)
	}

	var tipHashes = make([]*chainhash.Hash, len(parentNames))
	var parents = make([]*wire.MsgBlock, len(parentNames))
	for i, parentName := range parentNames {
		parents[i] = g.blocksByName[parentName]
	}

	for i, tip := range parents {
		hash := tip.BlockHash()
		tipHashes[i] = &hash
	}
	//fmt.Printf("tips hashes: %v\n", tipHashes)
	prevHash := blockdag.GenerateTipsHash(tipHashes)

	parentInfo := make([]*wire.Parent, len(tipHashes))
	for i, parent := range parents {
		parentInfo[i] = &wire.Parent{
			Hash: parent.BlockHash(),
		}
	}
	//fmt.Printf("parent info: %v\n", parentInfo)
	block := wire.MsgBlock{
		...
		Parents: wire.ParentSubHeader {
			Version:  1,
			Size:     int32(len(parentInfo)),
			Parents:  parentInfo,

		},
		Transactions: txns,
	}
	//fmt.Printf("block done\n")
	...
	//fmt.Printf("current tips: %v\n", g.tipNames)
	//fmt.Printf("parents: %v\n", parentNames)

	// remove parents from tip list
	for _, tip := range g.tipNames {
		includeTip := true
	...
	newTips = append(newTips, blockName)
	g.tipNames = newTips
	//fmt.Printf("updated tips: %v \n", newTips)
	return &block
}
...
// blocks.
func (g *testGenerator) setLast(blockName string) {
	g.last = g.blocksByName[blockName]
	g.lastName = blockName
	g.lastHeight = g.blockHeights[blockName]
}

func (g *testGenerator) setTips(tipNames []string) {
	g.tipNames = tipNames
}
...
func (g *testGenerator) saveTipCoinbaseOut() {
	g.spendableOuts = append(g.spendableOuts, makeSpendableOut(g.last, 0, 0))
	g.prevCollectedHash = g.last.BlockHash()
}
...
func (g *testGenerator) saveSpendableCoinbaseOuts() {
	...
	curTipName := g.lastName
	defer g.setLast(curTipName)

	tipBlocks := make([]*wire.MsgBlock, len(g.tipNames))
	for i, tipName := range g.tipNames {
		tipBlocks[i] = g.blocksByName[tipName]
	}
	...
	for b := tipBlocks; b != nil; { //b = g.blocks[b.Header.PrevBlock] {
		foundHash := false
		nextBlocks := make([]*wire.MsgBlock,0)
		for _, block := range b {
			if block.BlockHash() == g.prevCollectedHash {
				foundHash = true
			} else {
				collectBlocks = append(collectBlocks, block)
				for _, parent := range block.Parents.Parents {
					parentBlock := g.blocks[parent.Hash]
					nextBlocks = append(nextBlocks, parentBlock)
				}
			}
		}
	...
	for i := range collectBlocks {
		g.last = collectBlocks[len(collectBlocks)-1-i]
...
func encodeNonCanonicalBlock(b *wire.MsgBlock) []byte {
	...
	b.Header.SotoEncode(&buf, 0, wire.BaseEncoding)
	buf.Write(nonCanonicalVarInt(uint32(len(b.Transactions))))
	for _, tx := range b.Transactions {
		tx.SotoEncode(&buf, 0, wire.BaseEncoding)
	}
	return buf.Bytes()
}
...
func (g *testGenerator) assertTipBlockSigOpsCount(expected int) {
	numSigOps := countBlockSigOps(g.last)
	if numSigOps != expected {
		panic(fmt.Sprintf("generated number of sigops for block %q "+
			"(height %d) is %d instead of expected %d", g.lastName,
			g.lastHeight, numSigOps, expected))
	}
}
...
func (g *testGenerator) assertTipBlockSize(expected int) {
	serializeSize := g.last.SerializeSize()
	if serializeSize != expected {
		panic(fmt.Sprintf("block size of block %q (height %d) is %d "+
			"instead of expected %d", g.lastName, g.lastHeight,
...
func (g *testGenerator) assertTipNonCanonicalBlockSize(expected int) {
	serializeSize := len(encodeNonCanonicalBlock(g.last))
	if serializeSize != expected {
		panic(fmt.Sprintf("block size of block %q (height %d) is %d "+
			"instead of expected %d", g.lastName, g.lastHeight,
			serializeSize, expected))
...
func (g *testGenerator) assertTipBlockNumTxns(expected int) {
	numTxns := len(g.last.Transactions)
	if numTxns != expected {
		panic(fmt.Sprintf("number of txns in block %q (height %d) is "+
			"%d instead of expected %d", g.lastName, g.lastHeight,
			numTxns, expected))
	}
}
...
func (g *testGenerator) assertTipBlockHash(expected chainhash.Hash) {
	hash := g.last.BlockHash()
	if hash != expected {
		panic(fmt.Sprintf("block hash of block %q (height %d) is %v "+
			"instead of expected %v", g.lastName, g.lastHeight, hash,
			expected))
	}
}
...
func (g *testGenerator) assertTipBlockTxOutOpReturn(txIndex, txOutIndex uint32) {
	if txIndex >= uint32(len(g.last.Transactions)) {
		panic(fmt.Sprintf("Transaction index %d in block %q "+
			"(height %d) does not exist", txIndex, g.lastName,
			g.lastHeight))
	}

	tx := g.last.Transactions[txIndex]
...
func Generate(includeLargeReorg bool) (tests [][]TestInstance, err error) {
	...
	//		ets[i] = &ExpectedTip{tipName, g.blocksByName[tipName], g.blockHeights[tipName]}
	//	}
	//	return ExpectedTips{ ets}
	//}
	...
	accepted := func() {
		tests = append(tests, []TestInstance{
			acceptBlock(g.lastName, g.last, true, false),
		})
	}
	//expectedTips := func(tips []string) {
	//	tests = append(tests, []TestInstance{
	//		expectTipBlocks(tips),
	//	})
	//}
	...
	rejected := func(code blockdag.ErrorCode) {
		tests = append(tests, []TestInstance{
			rejectBlock(g.lastName, g.last, code),
		})
	}
	...
	for i := uint16(0); i < coinbaseMaturity; i++ {
		...
		g.nextBlock(blockName, nil, []string{g.lastName})
		g.saveTipCoinbaseOut()
		testInstances = append(testInstances, acceptBlock(g.lastName,
			g.last, true, false))
	}
	...
	//
	//   ... -> b1(0) -> b2(1)
	g.nextBlock("b1", outs[0], []string{g.lastName})
	accepted()


	g.nextBlock("b2", outs[1], []string{"b1"})
	accepted()

	// TODO: once we have spending/coloring sorting
	// ---------------------------------------------------------------------
	// Double spend tests.
	// ---------------------------------------------------------------------
	...
	//                                         \-> b9(4)
	//               \-> b3(1) -> b4(2)
	g.nextBlock("b3", outs[2], []string{"b2"}, additionalCoinbase(1))
	rejected(blockdag.ErrBadCoinbaseValue)


	// TODO: enable once we have txs and spending
	// ---------------------------------------------------------------------
	...
	g.nextBlock("b4", outs[2], []string{"b2"}, replaceSpendScript(manySigOps))
	...
	g.nextBlock("b5", outs[3], []string{"b4"}, replaceSpendScript(tooManySigOps))
	...
	g.setLast("b4")
    g.setTips([]string{"b4"})
	g.nextBlock("b7", outs[3], nil, func(b *wire.MsgBlock) {
