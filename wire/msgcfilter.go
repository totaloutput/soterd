...
// MsgCFilter implements the Message interface and represents a soter cfilter
...
func (msg *MsgCFilter) SotoDecode(r io.Reader, pver uint32, _ MessageEncoding) error {
	// Read filter type
...
// This is part of the Message interface implementation.
func (msg *MsgCFilter) SotoEncode(w io.Writer, pver uint32, _ MessageEncoding) error {
	size := len(msg.Data)
...
// from SotoDecode in that SotoDecode decodes from the soter wire protocol as
...
// NewMsgCFilter returns a new soter cfilter message that conforms to the
