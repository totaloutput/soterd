...
const (
	...
	// that can be in a single soter cfheaders message.
	MaxCFHeadersPerMsg = 2000
)

// MsgCFHeaders implements the Message interface and represents a soter
...
func (msg *MsgCFHeaders) SotoDecode(r io.Reader, pver uint32, _ MessageEncoding) error {
	// Read filter type
...
// This is part of the Message interface implementation.
func (msg *MsgCFHeaders) SotoEncode(w io.Writer, pver uint32, _ MessageEncoding) error {
	// Write filter type
...
// NewMsgCFHeaders returns a new soter cfheaders message that conforms to
