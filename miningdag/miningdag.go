...
const (
	...
	MinHighPriority = soterutil.NanoSoterPerSoter * 144.0 / 250

	// blockHeaderOverhead is the max number of bytes it takes to serialize
	// a block header, parent sub-header, and max possible transaction count.
	blockHeaderOverhead = wire.MaxBlockHeaderPayload + wire.MaxParentSubHeaderPayload + wire.MaxVarIntPayload

	// CoinbaseFlags is added to the coinbase script of a generated block
	// and is used to monitor BIP16 support as well as blocks that are
	// generated via soterd.
	CoinbaseFlags = "/P2SH/soterd/"
)
...
type TxDesc struct {
	...
	// FeePerKB is the fee the transaction pays in nanoSoter per 1000 bytes.
	FeePerKB int64
}
...
func createCoinbaseTx(params *chaincfg.Params, coinbaseScript []byte, nextBlockHeight int32, addr soterutil.Address) (*soterutil.Tx, error) {
...
func spendTransaction(utxoView *blockdag.UtxoViewpoint, tx *soterutil.Tx, height int32) error {
...
func logSkippedDeps(tx *soterutil.Tx, deps map[chainhash.Hash]*txPrioItem) {
	if deps == nil {
		return
	}
...
func NewBlkTmplGenerator(policy *Policy, params *chaincfg.Params,
	txSource TxSource, chain *blockdag.BlockDAG,
	timeSource blockdag.MedianTimeSource,
	sigCache *txscript.SigCache,
...
func (g *BlkTmplGenerator) NewBlockTemplate(payToAddress soterutil.Address) (*BlockTemplate, error) {
	...
	nextBlockHeight := snapshot.MaxHeight + 1
...
mempoolLoop:
	...
	segwitStates, err := g.chain.ThresholdStates(chaincfg.DeploymentSegwit)
	if err != nil {
		return nil, err
	}
	segwitActiveCount := 0
	for _, segwitState := range segwitStates {
		if segwitState == blockdag.ThresholdActive {
			segwitActiveCount++
		}
	}
	segwitActive := segwitActiveCount == len(segwitStates)
	...
	for priorityQueue.Len() > 0 {
		...
		case segwitActive && !witnessIncluded && tx.HasWitness():
			...
			// witness commitment.
			coinbaseCopy := soterutil.NewTx(coinbaseTx.MsgTx().Copy())
			coinbaseCopy.MsgTx().TxIn[0].Witness = [][]byte{
				bytes.Repeat([]byte("a"),
					blockdag.CoinbaseWitnessDataLen),
			}
			coinbaseCopy.MsgTx().AddTxOut(&wire.TxOut{
				PkScript: bytes.Repeat([]byte("a"),
					blockdag.CoinbaseWitnessPkScriptLength),
			})
			...
			weightDiff := blockdag.GetTransactionWeight(coinbaseCopy) -
				blockdag.GetTransactionWeight(coinbaseTx)
		...
		txWeight := uint32(blockdag.GetTransactionWeight(tx))
		...
		// check for overflow.
		sigOpCost, err := blockdag.GetSigOpCost(tx, false,
		...
		if blockSigOpCost+int64(sigOpCost) < blockSigOpCost ||
			blockSigOpCost+int64(sigOpCost) > blockdag.MaxBlockSigOpsCost {
		...
		_, err = blockdag.CheckTransactionInputs(tx, nextBlockHeight, blockUtxos, g.chainParams, false)
		if err != nil {
			log.Tracef("Skipping tx %s due to error in "+
	...
	if witnessIncluded {
		...
		// of all zeroes.
		var witnessNonce [blockdag.CoinbaseWitnessDataLen]byte
		...
		witnessMerkleTree := blockdag.BuildMerkleTreeStore(blockTxns,
			true)
		...
		witnessScript := append(blockdag.WitnessMagicBytes, witnessCommitment...)
	...
	// PrevBlock in msgBlock.Header should be the hash of the DAG tips
	...
	// Add the parent sub-header to the block
	nextParentVersion, err := g.chain.CalcNextParentVersion()
	if err != nil {
		return nil, err
	}
...
func (g *BlkTmplGenerator) UpdateExtraNonce(msgBlock *wire.MsgBlock, blockHeight int32, extraNonce uint64) error {
	...
	// TODO(davec): A soterutil.Block should use saved in the state to avoid
	// recalculating all of the other transaction hashes.
	// block.Transactions[0].InvalidateCache()

	// Recalculate the merkle root with the updated extra nonce.
	block := soterutil.NewBlock(msgBlock)
	merkles := blockdag.BuildMerkleTreeStore(block.Transactions(), false)
...
func (g *BlkTmplGenerator) DAGSnapshot() *blockdag.DAGState {
