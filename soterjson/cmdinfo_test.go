...
func TestCmdMethod(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			cmd:    (*soterjson.GetBlockCmd)(nil),
			method: "getblock",
		},
		{
			name:   "nil instance of registered type",
			cmd:    &soterjson.GetBlockCountCmd{},
	...
	for i, test := range tests {
		method, err := soterjson.CmdMethod(test.cmd)
		...
		if err != nil {
			...
			if gotErrorCode != test.err.(soterjson.Error).ErrorCode {
				t.Errorf("Test #%d (%s) mismatched error code "+
					"- got %v (%v), want %v", i, test.name,
					gotErrorCode, err,
					test.err.(soterjson.Error).ErrorCode)
				continue
			}
...
func TestMethodUsageFlags(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			flags:  soterjson.UFWalletOnly,
		},
	}

	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		flags, err := soterjson.MethodUsageFlags(test.method)
		if reflect.TypeOf(err) != reflect.TypeOf(test.err) {
			t.Errorf("Test #%d (%s) wrong error - got %T (%[3]v), "+
				"want %T", i, test.name, err, test.err)
			continue
...
func TestMethodUsageText(t *testing.T) {
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		usage, err := soterjson.MethodUsageText(test.method)
		if reflect.TypeOf(err) != reflect.TypeOf(test.err) {
			t.Errorf("Test #%d (%s) wrong error - got %T (%[3]v), "+
				"want %T", i, test.name, err, test.err)
			continue
...
func TestFieldUsage(t *testing.T) {
	...
	for i, test := range tests {
		// Ensure usage matches the expected value.
		usage := soterjson.TstFieldUsage(test.field, test.defValue)
		if usage != test.expected {
			t.Errorf("Test #%d (%s) mismatched usage - got %v, "+
				"want %v", i, test.name, usage, test.expected)
			continue
