...
func TestIsValidIDType(t *testing.T) {
	...
	for i, test := range tests {
		if soterjson.IsValidIDType(test.id) != test.isValid {
...
func TestMarshalResponse(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			result: nil,
			jsonErr: func() *soterjson.RPCError {
				return soterjson.NewRPCError(soterjson.ErrRPCBlockNotFound, "123 not found")
			}(),
	...
	for i, test := range tests {
		_, _ = i, test
		marshalled, err := soterjson.MarshalResponse(testID, test.result, test.jsonErr)
		if err != nil {
...
func TestMiscErrors(t *testing.T) {
	...
	// not supported.
	_, err := soterjson.NewRequest(nil, "test", []interface{}{make(chan int)})
	if err == nil {
	...
	// supported.
	wantErr := soterjson.Error{ErrorCode: soterjson.ErrInvalidType}
	_, err = soterjson.MarshalResponse(make(chan int), nil, nil)
	if jerr, ok := err.(soterjson.Error); !ok || jerr.ErrorCode != wantErr.ErrorCode {
	...
	// can't be marshalled.
	_, err = soterjson.MarshalResponse(1, make(chan int), nil)
...
func TestRPCError(t *testing.T) {
	...
	tests := []struct {
		...
		{
			soterjson.ErrRPCMethodNotFound,
