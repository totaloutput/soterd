...
var (
	...
	// data.
	gbtCoinbaseAux = &soterjson.GetBlockTemplateResultAux{
		Flags: hex.EncodeToString(builderScript(txscript.
			NewScriptBuilder().
			AddData([]byte(miningdag.CoinbaseFlags)))),
	}
...
var (
	...
	ErrRPCUnimplemented = &soterjson.RPCError{
	...
	ErrRPCNoWallet = &soterjson.RPCError{
		Code:    soterjson.ErrRPCNoWallet,
...
// should ask a connected instance of soterwallet.
...
// not needed.
func internalRPCError(errStr, context string) *soterjson.RPCError {
	logStr := errStr
	if context != "" {
		logStr = context + ": " + errStr
	}
	rpcsLog.Error(logStr)
	return soterjson.NewRPCError(soterjson.ErrRPCInternal.Code, errStr)
}

// rpcDecodeHexError is a convenience function for returning a nicely formatted
// RPC error which indicates the provided hex string failed to decode.
func rpcDecodeHexError(gotHex string) *soterjson.RPCError {
	return soterjson.NewRPCError(soterjson.ErrRPCDecodeHexString,
...
// transaction hash.
func rpcNoTxInfoError(txHash *chainhash.Hash) *soterjson.RPCError {
	return soterjson.NewRPCError(soterjson.ErrRPCNoTxInfo,
...
// These commands will be implemented in soterwallet.
...
func messageToHex(msg wire.Message) (string, error) {
	...
	if err := msg.SotoEncode(&buf, maxProtocolVersion, wire.WitnessEncoding); err != nil {
...
func handleCreateRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*soterjson.CreateRawTransactionCmd)
	...
	for encodedAddr, amount := range c.Amounts {
		...
		if amount <= 0 || amount > soterutil.MaxNanoSoter {
			return nil, &soterjson.RPCError{
				Code:    soterjson.ErrRPCType,
				Message: "Invalid amount",
			}
		}

		// Decode the provided address.
		addr, err := soterutil.DecodeAddress(encodedAddr, params)
		if err != nil {
			return nil, &soterjson.RPCError{
		...
		// Convert the amount to nanoSoter.
		nanoSoter, err := soterutil.NewAmount(amount)
		if err != nil {
			context := "Failed to convert amount"
			return nil, internalRPCError(err.Error(), context)
		}

		txOut := wire.NewTxOut(int64(nanoSoter), pkScript)
		mtx.AddTxOut(txOut)
	}
...
// transaction.
func createVinList(mtx *wire.MsgTx) []soterjson.Vin {
	// Coinbase transactions only have a single txin by definition.
	vinList := make([]soterjson.Vin, len(mtx.TxIn))
	...
	for i, txIn := range mtx.TxIn {
		...
		vinEntry.ScriptSig = &soterjson.ScriptSig{
			Asm: disbuf,
...
// transaction.
func createVoutList(mtx *wire.MsgTx, chainParams *chaincfg.Params, filterAddrMap map[string]struct{}) []soterjson.Vout {
	voutList := make([]soterjson.Vout, 0, len(mtx.TxOut))
...
func createTxRawResult(chainParams *chaincfg.Params, mtx *wire.MsgTx,
	...
	blkHeight int32, chainHeight int32) (*soterjson.TxRawResult, error) {
...
func handleDecodeRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*soterjson.DecodeRawTransactionCmd)
	...
	txReply := soterjson.TxRawDecodeResult{
...
func handleDecodeScript(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	p2sh, err := soterutil.NewAddressScriptHash(script, s.cfg.ChainParams)
	if err != nil {
	...
	reply := soterjson.DecodeScriptResult{
		Asm:       disbuf,
...
func handleEstimateFee(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*soterjson.EstimateFeeCmd)
...
func handleGetAddedNodeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if c.Node != nil {
		...
		if !found {
			return nil, &soterjson.RPCError{
				Code:    soterjson.ErrRPCClientNodeNotAdded,
	...
	results := make([]*soterjson.GetAddedNodeInfoResult, 0, len(peers))
	for _, rpcPeer := range peers {
		// Set the "address" of the peer which could be an ip address
		// or a domain name.
		peer := rpcPeer.ToPeer()
		var result soterjson.GetAddedNodeInfoResult
		result.AddedNode = peer.Addr()
		result.Connected = soterjson.Bool(peer.Connected())
		...
		addrs := make([]soterjson.GetAddedNodeInfoResultAddr, 0, len(ipList))
		for _, ip := range ipList {
			var addr soterjson.GetAddedNodeInfoResultAddr
			addr.Address = ip
...
// handleGetAddrCache implements the getaddrcache RPC call, which invokes the getaddrcache p2p call and returns all
// known addresses for all peers.
func handleGetAddrCache(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	// Ask all peers what addresses they have available
...
func handleGetBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	var nextHashesStrings []string
	if blockHeight < dagState.MaxHeight {
		nextHashes, err := s.cfg.Chain.BlockHashesByHeight(blockHeight + 1)
		if err != nil {
			context := "No next block"
			return nil, internalRPCError(err.Error(), context)
		}
		for _, nextHash := range nextHashes {
			nextHashesStrings = append(nextHashesStrings, nextHash.String())
		}
	}

	params := s.cfg.ChainParams
	blockHeader := &blk.MsgBlock().Header
	blockReply := soterjson.GetBlockVerboseResult{
		Hash:          c.Hash,
		...
		Confirmations: int64(1 + dagState.MaxHeight - blockHeight),
		Height:        int64(blockHeight),
		Size:          int32(len(blkBytes)),
		StrippedSize:  int32(blk.MsgBlock().SerializeSizeStripped()),
		Weight:        int32(blockdag.GetBlockWeight(blk)),
	...
	if len(parents) > 0 {
		dagParents := make([]soterjson.DAGParent, len(parents))
		for i, parent := range parents {
			dagParents[i] = soterjson.DAGParent{
				Version: parentSubHeader.Version,
				Hash:    parent.Hash.String(),
				Data:    parent.Data,
			}
		}
		blockReply.Parents = dagParents
	}
	...
	if c.VerboseTx == nil || !*c.VerboseTx {
		...
		rawTxns := make([]soterjson.TxRawResult, len(txns))
		for i, tx := range txns {
			rawTxn, err := createTxRawResult(params, tx.MsgTx(),
				tx.Hash().String(), blockHeader, hash.String(),
				blockHeight, dagState.MaxHeight)
			if err != nil {
				return nil, err
			}
			rawTxns[i] = *rawTxn
...
// softForkStatuses converts []ThresholdState into a human readable string
// corresponding to a particular state.
func softForkStatuses(statuses []blockdag.ThresholdState) ([]string, error) {
	var statusStrings []string
	for _, status := range statuses {
		statusString, err := softForkStatus(status)
		if err != nil {
			return nil, &soterjson.RPCError{
				Code: soterjson.ErrRPCInternal.Code,
		...
		statusStrings = append(statusStrings, strings.ToLower(statusString))
	}
	return statusStrings, nil
}
...
func handleGetBlockChainInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	dagState := chain.DAGSnapshot()

	chainInfo := &soterjson.GetBlockChainInfoResult{
		Chain:         params.Name,
		Blocks:        int32(dagState.BlkCount),
		Headers:       int32(dagState.BlkCount),
		BestBlockHash: chainSnapshot.Hash.String(), //TODO(jenlouie): send tips instead?
		Difficulty:    getDifficultyRatio(chainSnapshot.Bits, params),
		MedianTime:    chainSnapshot.MedianTime.Unix(),
		Pruned:        false,
		Bip9SoftForks: make(map[string]*soterjson.Bip9SoftForkDescription),
	}
	...
	height := dagState.MaxHeight
	chainInfo.SoftForks = []*soterjson.SoftForkDescription{
		{
			ID:      "bip34",
			Version: 2,
			Reject: struct {
	...
	for deployment, deploymentDetails := range params.Deployments {
		...
		deploymentStatuses, err := chain.ThresholdStates(uint32(deployment))
		if err != nil {
			context := "Failed to obtain deployment status"
			return nil, internalRPCError(err.Error(), context)
		}

		// Attempt to convert the current deployment statuses into a
		// human readable string. If the status is unrecognized, then a
		// non-nil error is returned.
		statusesString, err := softForkStatuses(deploymentStatuses)

		// Finally, populate the soft-fork description with all the
		// information gathered above.
		chainInfo.Bip9SoftForks[forkName] = &soterjson.Bip9SoftForkDescription{
...
func handleGetBlockCount(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	dagState := s.cfg.Chain.DAGSnapshot()
	return int64(dagState.BlkCount), nil
}

// handleGetBlockHash implements the getblockhash command.
func handleGetBlockHash(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*soterjson.GetBlockHashCmd)
	hashes, err := s.cfg.Chain.BlockHashesByHeight(int32(c.Index))
	if err != nil {
		return nil, &soterjson.RPCError{
...
func handleGetBlockHeader(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if blockHeight < dagState.MaxHeight {
		...
		for _, hash := range nextHashes {
			nextHashStrings = append(nextHashStrings, hash.String())
		}
	}

	params := s.cfg.ChainParams
	blockHeaderReply := soterjson.GetBlockHeaderVerboseResult{
		Hash:          c.Hash,
		Confirmations: int64(1 + dagState.MaxHeight - blockHeight),
		Height:        blockHeight,
		Version:       blockHeader.Version,
...
func (state *gbtWorkState) updateBlockTemplate(s *rpcServer, useCoinbaseValue bool) error {
	...
	// jenlouie: set latestHash to be latest tips hash instead of latest block hash
	latestHash := &s.cfg.Chain.DAGSnapshot().Hash
	...
	if template == nil || state.prevHash == nil ||
		...
		// consensus rules.
		best := s.cfg.Chain.BestSnapshot()
		minTimestamp := miningdag.MinimumMedianTime(best)
...
// state as a soterjson.GetBlockTemplateResult that is ready to be encoded to JSON
// and returned to the caller.
//
// This function MUST be called with the state locked.
func (state *gbtWorkState) blockTemplateResult(useCoinbaseValue bool, submitOld *bool) (*soterjson.GetBlockTemplateResult, error) {
	...
	transactions := make([]soterjson.GetBlockTemplateResultTx, 0, numTx-1)
	...
	for i, tx := range msgBlock.Transactions {
		...
		bTx := soterutil.NewTx(tx)
		resultTx := soterjson.GetBlockTemplateResultTx{
			Data:    hex.EncodeToString(txBuf.Bytes()),
			Hash:    txHash.String(),
			Depends: depends,
			Fee:     template.Fees[i],
			SigOps:  template.SigOpCosts[i],
			Weight:  blockdag.GetTransactionWeight(bTx),
		}
...
func handleGetBlockTemplateRequest(s *rpcServer, request *soterjson.TemplateRequest, closeChan <-chan struct{}) (interface{}, error) {
	...
	if !(cfg.RegressionTest || cfg.SimNet) &&
		...
		return nil, &soterjson.RPCError{
			Code:    soterjson.ErrRPCClientNotConnected,
			Message: "Soter is not connected",
		}
	}

	// No point in generating or accepting work before the chain is synced.
	currentHeight := s.cfg.Chain.DAGSnapshot().MaxHeight
...
// chainErrToGBTErrString converts an error returned from blockdag to a string
...
func chainErrToGBTErrString(err error) string {
	...
	case blockdag.ErrBlockWeightTooHigh:
	...
	case blockdag.ErrUnexpectedWitness:
		return "unexpected-witness"
	case blockdag.ErrInvalidWitnessCommitment:
		return "bad-witness-nonce-size"
	case blockdag.ErrWitnessCommitmentMismatch:
...
func handleGetBlockTemplateProposal(s *rpcServer, request *soterjson.TemplateRequest) (interface{}, error) {
	hexData := request.Data
	if hexData == "" {
		return false, &soterjson.RPCError{
			Code: soterjson.ErrRPCType,
	...
	// Ensure the block is building from the expected set of tips.
	// In DAG, PrevBlock is hash of tips
	expectedPrevHash := s.cfg.Chain.DAGSnapshot().Hash
...
func handleGetBlockTemplate(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*soterjson.GetBlockTemplateCmd)
	request := c.Request
...
// handleGetBlockMetrics implements the getblockmetrics RPC call, which returns how long it took to generate the
// latest blocks in milliseconds
func handleGetBlockMetrics(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	// 1ms = 10^6 ns
	nsToMS := math.Pow10(6)

	// Return the block time duration in milliseconds
	var msTimes []float64
	for _, d := range s.cfg.MetricsMgr.MinerSolveTimes() {
		ms := float64(d.Nanoseconds()) / nsToMS
		msTimes = append(msTimes, ms)
	}
...
//
// NOTE: This is a soterd extension originally ported from
// github.com/decred/dcrd.
func handleGetHeaders(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*soterjson.GetHeadersCmd)

	// Fetch the requested headers from chain while respecting the provided
	// block locators and stop hash.
	blockLocators := make([]*int32, len(c.BlockLocators))
	for i := range c.BlockLocators {
		blockLocator64, err := strconv.ParseInt(c.BlockLocators[i], 10, 32)
		blockLocator := int32(blockLocator64)
		if err != nil {
...
// handleGetListenAddrs implements the getlistenaddrs command, which returns a list of p2p addresses the server is listening on.
func handleGetListenAddrs(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	addrs := s.cfg.ConnMgr.ListenAddrs()

	result := &soterjson.GetListenAddrsResult{
		P2P: addrs,
	}
...
func handleGetMempoolInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	ret := &soterjson.GetMempoolInfoResult{
...
func handleGetMiningInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	gnhpsCmd := soterjson.NewGetNetworkHashPSCmd(nil, nil)
	...
	best := s.cfg.Chain.BestSnapshot()
	dagState := s.cfg.Chain.DAGSnapshot()
	result := soterjson.GetMiningInfoResult{
		Blocks:             int64(dagState.BlkCount),
...
func handleGetNetTotals(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	reply := &soterjson.GetNetTotalsResult{
...
func handleGetNetworkHashPS(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*soterjson.GetNetworkHashPSCmd)
	...
	if endHeight > dagState.MaxHeight || endHeight == 0 {
		return int64(0), nil
	}
	if endHeight < 0 {
		endHeight = dagState.MaxHeight
	}
	...
	for curHeight := startHeight; curHeight <= endHeight; curHeight++ {
		hashes, err := s.cfg.Chain.BlockHashesByHeight(curHeight)
		if err != nil {
			context := "Failed to fetch block hash"
			return nil, internalRPCError(err.Error(), context)
		}

		// Fetch the headers from chain.
		for _, hash := range hashes {
...
func handleGetPeerInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	infos := make([]*soterjson.GetPeerInfoResult, 0, len(peers))
	for _, p := range peers {
		statsSnap := p.ToPeer().StatsSnapshot()
		info := &soterjson.GetPeerInfoResult{
			ID:             statsSnap.ID,
			Addr:           statsSnap.Addr,
...
func handleGetRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*soterjson.GetRawTransactionCmd)
	...
	if blkHash != nil {
		...
		chainHeight = s.cfg.Chain.DAGSnapshot().MaxHeight
	}
...
func handleGetTxOut(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if includeMempool && s.cfg.TxMemPool.HaveTransaction(txHash) {
		...
		if c.Vout > uint32(len(mtx.TxOut)-1) {
			return nil, &soterjson.RPCError{
				Code: soterjson.ErrRPCInvalidTxVout,
		...
		dagState := s.cfg.Chain.DAGSnapshot()
		bestBlockHash = best.Hash.String()
		confirmations = 1 + dagState.MaxHeight - entry.BlockHeight()
		value = entry.Amount()
...
// handleRenderDag implements the renderdag RPC call.
// It returns a rendered dag in graphviz DOT file format
func handleRenderDag(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {

	dot, err := s.cfg.Chain.RenderDot()
	if err != nil {
		return nil, err
	}

	result := &soterjson.RenderDagResult{
		Dot: fmt.Sprintf("%s", dot),
	}
...
// passed transaction.
func createVinListPrevOut(s *rpcServer, mtx *wire.MsgTx, chainParams *chaincfg.Params, vinExtra bool, filterAddrMap map[string]struct{}) ([]soterjson.VinPrevOut, error) {
	// Coinbase transactions only have a single txin by definition.
	if blockdag.IsCoinBaseTx(mtx) {
		...
		txIn := mtx.TxIn[0]
		vinList := make([]soterjson.VinPrevOut, 1)
		vinList[0].Coinbase = hex.EncodeToString(txIn.SignatureScript)
		vinList[0].Sequence = txIn.Sequence
		return vinList, nil
	}
	...
	vinList := make([]soterjson.VinPrevOut, 0, len(mtx.TxIn))
...
func fetchMempoolTxnsForAddress(s *rpcServer, addr soterutil.Address, numToSkip, numRequested uint32) ([]*soterutil.Tx, uint32) {
...
func handleSearchRawTransactions(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*soterjson.SearchRawTransactionsCmd)
	vinExtra := false
	if c.VinExtra != nil {
	...
	params := s.cfg.ChainParams
	addr, err := soterutil.DecodeAddress(c.Address, params)
	if err != nil {
		return nil, &soterjson.RPCError{
			Code:    soterjson.ErrRPCInvalidAddressOrKey,
			Message: "Invalid address or key: " + err.Error(),
	...
	//best := s.cfg.Chain.BestSnapshot()
	dagState := s.cfg.Chain.DAGSnapshot()
	srtList := make([]soterjson.SearchRawTransactionsResult, len(addressTxns))
	for i := range addressTxns {
		...
		if blkHeader != nil {
			...
			result.Confirmations = uint64(1 + dagState.MaxHeight - blkHeight)
		}
	}
...
func handleSendRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*soterjson.SendRawTransactionCmd)
...
func handleSetGenerate(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*soterjson.SetGenerateCmd)
...
func handleValidateAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*soterjson.ValidateAddressCmd)

	result := soterjson.ValidateAddressChainResult{}
	addr, err := soterutil.DecodeAddress(c.Address, s.cfg.ChainParams)
	if err != nil {
...
func verifyChain(s *rpcServer, level, depth int32) error {
	//best := s.cfg.Chain.BestSnapshot()
	dagState := s.cfg.Chain.DAGSnapshot()
	finishHeight := dagState.MaxHeight - depth
	if finishHeight < 0 {
		finishHeight = 0
	}
	rpcsLog.Infof("Verifying chain for %d blocks at level %d",
		dagState.MaxHeight-finishHeight, level)

	for height := dagState.MaxHeight; height > finishHeight; height-- {
		// Level 0 just looks up the block.
		blocks, err := s.cfg.Chain.BlocksByHeight(height)
		if err != nil {
...
func handleVerifyChain(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*soterjson.VerifyChainCmd)
...
func handleVerifyMessage(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*soterjson.VerifyMessageCmd)
	...
	if _, ok := addr.(*soterutil.AddressPubKeyHash); !ok {
		return nil, &soterjson.RPCError{
			Code:    soterjson.ErrRPCType,
	...
	address, err := soterutil.NewAddressPubKey(serializedPK, params)
	if err != nil {
...
//
// NOTE: This is a soterd extension ported from github.com/decred/dcrd.
func handleVersion(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	result := map[string]soterjson.VersionResult{
		"soterdjsonrpcapi": {
...
type parsedRPCCmd struct {
	...
	method string
	cmd    interface{}
	err    *soterjson.RPCError
}

// standardCmdResult checks that a parsed command is a standard Soter JSON-RPC
...
func (s *rpcServer) standardCmdResult(cmd *parsedRPCCmd, closeChan <-chan struct{}) (interface{}, error) {
	...
	return nil, soterjson.ErrRPCMethodNotFound
handled:
...
func parseCmd(request *soterjson.Request) *parsedRPCCmd {
	...
	if err != nil {
		...
		if jerr, ok := err.(soterjson.Error); ok &&
			jerr.ErrorCode == soterjson.ErrUnregisteredMethod {

			parsedCmd.err = soterjson.ErrRPCMethodNotFound
			return &parsedCmd
		}

		// Otherwise, some type of invalid parameters is the
		// cause, so produce the equivalent RPC error.
		parsedCmd.err = soterjson.NewRPCError(
			soterjson.ErrRPCInvalidParams.Code, err.Error())
		return &parsedCmd
	}
...
// the type *soterjson.RPCError to the appropriate type as needed.
func createMarshalledReply(id, result interface{}, replyErr error) ([]byte, error) {
	var jsonErr *soterjson.RPCError
	if replyErr != nil {
		if jErr, ok := replyErr.(*soterjson.RPCError); ok {
			jsonErr = jErr
		} else {
	...
	return soterjson.MarshalResponse(id, result, jsonErr)
}
...
func jsonAuthFail(w http.ResponseWriter) {
	w.Header().Add("WWW-Authenticate", `Basic realm="soterd RPC"`)
...
// The interface contract requires that all of these methods are safe for
// concurrent access.
type rpcserverMetricsManager interface {
	// Returns a counter for number of blocks mined by the CPUMiner
	MinerSolveCount() int64
	// Returns a cache of block hash strings for blocks mined by the CPUMiner
	MinerSolveHashes() []string
	// Returns a cache of solve times for blocks mined by the CPUMiner
	MinerSolveTimes() []time.Duration
}
...
type rpcserverSyncManager interface {
	...
	SubmitBlock(block *soterutil.Block, flags blockdag.BehaviorFlags) (bool, error)
...
type rpcserverConfig struct {
	...
	Generator *miningdag.BlkTmplGenerator
