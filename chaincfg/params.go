...
var (
	...
	// mainPowLimit is the highest proof of work value a soter block can
	// have for the main network.  It is the value 2^224 - 1.
	mainPowLimit = new(big.Int).Sub(new(big.Int).Lsh(bigOne, 224), bigOne)

	// regressionPowLimit is the highest proof of work value a soter block
	// can have for the regression test network.  It is the value 2^255 - 1.
	regressionPowLimit = new(big.Int).Sub(new(big.Int).Lsh(bigOne, 255), bigOne)

	// testNet1PowLimit is the highest proof of work value a soter block
	// can have for the test network (version 3).  It is the value
	// 2^224 - 1.
	testNet1PowLimit = new(big.Int).Sub(new(big.Int).Lsh(bigOne, 231), bigOne)

	// simNetPowLimit is the highest proof of work value a soter block
...
// Params defines a soter network by its parameters.  These parameters may be
// used by Soteria applications to differentiate networks as well as addresses
...
// MainNetParams defines the network parameters for the main soter network.
...
// soter network.  Not to be confused with the test soter network (version
...
// TestNet1Params defines the network parameters for the test soter network
...
var TestNet1Params = Params{
	...
	GenesisHash:              &testNet1GenesisHash,
	PowLimit:                 testNet1PowLimit,
	...
	CoinbaseMaturity:         100,
	SubsidyReductionInterval: 210000,
	TargetTimespan:           time.Hour * 24 * 1, // 1 day
...
// SimNetParams defines the network parameters for the simulation test soter
...
var (
	// ErrDuplicateNet describes an error where the parameters for a soter
	// network could not be set due to the network already being a standard
	// network or previously-registered into this package.
	ErrDuplicateNet = errors.New("duplicate Soter network")
...
var (
	registeredNets       = make(map[wire.SoterNet]struct{})
...
// Register registers the network parameters for a soter network.  This may
