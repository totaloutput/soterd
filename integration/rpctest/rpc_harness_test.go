...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt soterutil.Amount) *chainhash.Hash {
	...
	// input.
	txid := genSpend(soterutil.Amount(5 * soterutil.NanoSoterPerSoter))
	...
	txid = genSpend(soterutil.Amount(500 * soterutil.NanoSoterPerSoter))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testActiveHarnesses(r *Harness, t *testing.T) {
	...
	harness1, err := New(&chaincfg.SimNetParams, nil, nil, false)
	if err != nil {
		t.Fatal(err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(soterutil.NanoSoterPerSoter, pkScript)

	const numTxns = 5
	txns := make([]*soterutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
		...
		txns = append(txns, soterutil.NewTx(tx))
	}
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := soterutil.Amount(50 * soterutil.NanoSoterPerSoter)
...
func TestMain(m *testing.M) {
	var err error
	mainHarness, err = New(&chaincfg.SimNetParams, nil, nil, false)
	if err != nil {
