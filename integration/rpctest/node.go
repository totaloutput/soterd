...
func newConfig(prefix, certFile, keyFile string, extra []string, keepLogs bool) (*nodeConfig, error) {
	soterdPath, err := soterdExecutablePath()
	if err != nil {
		soterdPath = "soterd"
	}
...
// arguments returns an array of arguments that be used to launch the soterd
// process.
...
// command returns the exec.Cmd which will be used to start the soterd process.
...
// to the soterd process that is launched via Start().
...
// as the base for the log and data directories for soterd.
...
// start creates a new soterd process, and writes its pid in a file reserved for
...
// stop interrupts the running soterd process process, and waits until it exits
...
// shutdown terminates the running soterd process, and cleans up all
