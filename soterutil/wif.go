...
type WIF struct {
	...
	// netID is the soter network identifier byte used when
...
func NewWIF(privKey *soterec.PrivateKey, net *chaincfg.Params, compress bool) (*WIF, error) {
	if net == nil {
...
//    either testnet1 or the regression test network
...
func DecodeWIF(wif string) (*WIF, error) {
	...
	switch decodedLen {
	case 1 + soterec.PrivKeyBytesLen + 1 + 4:
		if decoded[33] != compressMagic {
			return nil, ErrMalformedPrivateKey
		}
		compress = true
	case 1 + soterec.PrivKeyBytesLen + 4:
		compress = false
	default:
		return nil, ErrMalformedPrivateKey
	}
	...
	var tosum []byte
	if compress {
		tosum = decoded[:1+soterec.PrivKeyBytesLen+1]
	} else {
		tosum = decoded[:1+soterec.PrivKeyBytesLen]
	}
	...
	netID := decoded[0]
	privKeyBytes := decoded[1 : 1+soterec.PrivKeyBytesLen]
	privKey, _ := soterec.PrivKeyFromBytes(soterec.S256(), privKeyBytes)
...
func (w *WIF) String() string {
	...
	// bytes of checksum.
	encodeLen := 1 + soterec.PrivKeyBytesLen + 4
	if w.CompressPubKey {
		encodeLen++
	}
	...
	a = paddedAppend(soterec.PrivKeyBytesLen, a, w.PrivKey.D.Bytes())
	if w.CompressPubKey {
...
func (w *WIF) SerializePubKey() []byte {
	pk := (*soterec.PublicKey)(&w.PrivKey.PublicKey)
	if w.CompressPubKey {
