...
func (k *ExtendedKey) pubKeyBytes() []byte {
	...
	// key if needed.
	if len(k.pubKey) == 0 {
		pkx, pky := soterec.S256().ScalarBaseMult(k.key)
		pubKey := soterec.PublicKey{Curve: soterec.S256(), X: pkx, Y: pky}
...
func (k *ExtendedKey) Child(i uint32) (*ExtendedKey, error) {
	...
	if ilNum.Cmp(soterec.S256().N) >= 0 || ilNum.Sign() == 0 {
	...
	if k.isPrivate {
		...
		ilNum.Mod(ilNum, soterec.S256().N)
		...
		ilx, ily := soterec.S256().ScalarBaseMult(il)
		...
		// public key.
		pubKey, err := soterec.ParsePubKey(k.key, soterec.S256())
		if err != nil {
			return nil, err
		}
		...
		//
		// childKey = serP(point(parse256(Il)) + parentKey)
		childX, childY := soterec.S256().Add(ilx, ily, pubKey.X, pubKey.Y)
		pk := soterec.PublicKey{Curve: soterec.S256(), X: childX, Y: childY}
		childKey = pk.SerializeCompressed()
	}

	// The fingerprint of the parent for the derived child is the first 4
	// bytes of the RIPEMD160(SHA256(parentPubKey)).
	parentFP := soterutil.Hash160(k.pubKeyBytes())[:4]
...
// ECPubKey converts the extended key to a soterec public key and returns it.
func (k *ExtendedKey) ECPubKey() (*soterec.PublicKey, error) {
	return soterec.ParsePubKey(k.pubKeyBytes(), soterec.S256())
}

// ECPrivKey converts the extended key to a soterec private key and returns it.
...
func (k *ExtendedKey) ECPrivKey() (*soterec.PrivateKey, error) {
	if !k.isPrivate {
		return nil, ErrNotPrivExtKey
	}

	privKey, _ := soterec.PrivKeyFromBytes(soterec.S256(), k.key)
	return privKey, nil
}

// Address converts the extended key to a standard soter pay-to-pubkey-hash
// address for the passed network.
func (k *ExtendedKey) Address(net *chaincfg.Params) (*soterutil.AddressPubKeyHash, error) {
	pkHash := soterutil.Hash160(k.pubKeyBytes())
	return soterutil.NewAddressPubKeyHash(pkHash, net)
}
...
func NewMaster(seed []byte, net *chaincfg.Params) (*ExtendedKey, error) {
	...
	if secretKeyNum.Cmp(soterec.S256().N) >= 0 || secretKeyNum.Sign() == 0 {
...
func NewKeyFromString(key string) (*ExtendedKey, error) {
	...
	if isPrivate {
		...
		if keyNum.Cmp(soterec.S256().N) >= 0 || keyNum.Sign() == 0 {
			return nil, ErrUnusableSeed
		}
	} else {
		// Ensure the public key parses correctly and is actually on the
		// secp256k1 curve.
		_, err := soterec.ParsePubKey(keyData, soterec.S256())
		if err != nil {
			return nil, err
		}
	}
