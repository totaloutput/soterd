...
// and netID which encodes the soter network and address type.  It is used
...
//
// The soter network the address is associated with is extracted if possible.
// When the address does not encode the network, such as in the case of a raw
// public key, the address will be associated with the passed defaultNet.
func DecodeAddress(addr string, defaultNet *chaincfg.Params) (Address, error) {
	// Bech32 encoded segwit addresses start with a human-readable part
	// (hrp) followed by '1'. For Soter mainnet the hrp is "bc", and for
...
func NewAddressPubKey(serializedPubKey []byte, net *chaincfg.Params) (*AddressPubKey, error) {
	pubKey, err := soterec.ParsePubKey(serializedPubKey, soterec.S256())
	if err != nil {
		return nil, err
	}

	// Set the format of the pubkey.  This probably should be returned
	// from soterec, but do it here to avoid API churn.  We already know the
...
// differs with the format.  At the time of this writing, most Soter addresses
...
func (a *AddressPubKey) PubKey() *soterec.PublicKey {
	return a.pubKey
}
