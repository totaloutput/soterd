...
func (cs *CoinSet) TotalValue() (value soterutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue soterutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue soterutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount soterutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue soterutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue soterutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue soterutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue soterutil.Amount, coins []Coin) (Coins, error) {
...
// soterutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() soterutil.Amount {
	return soterutil.Amount(c.txOut().Value)
}
